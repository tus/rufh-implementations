    ...
    private final class _Storage: @unchecked Sendable, Hashable {
        ...
        var ensureIndex: [String: UInt16] {
            ...
            for index in fields.indices.reversed() {
                let name = fields[index].0.name.canonicalName
                fields[index].1 = newIndex[name] ?? .max
...
extension HTTPFields: RangeReplaceableCollection, RandomAccessCollection, MutableCollection {
    ...
    public mutating func reserveCapacity(_ capacity: Int) {
        ...
        _storage.index?.reserveCapacity(capacity)
        _storage.fields.reserveCapacity(capacity)
    }
}
